...
class ImportUseCommand(sublime_plugin.TextCommand):
    def run(self, edit, namespace):
        ...
        if self.is_already_used():
            return self.view.show_popup('Already imported',
    ...
    def insert_use(self, edit):
        if self.is_first_use():
            for location in [r"^\s*namespace\s+[\w\\]+[;{]", r"^\s*declare\(strict_types=1\)", r"<\?php"]:
    ...
    def insert_first_use(self, where, edit):
        ...
        if not region.empty():
            ...
            self.view.show_popup('Imported ' + self.namespace,
                                 flags=sublime.HIDE_ON_MOUSE_MOVE_AWAY)
            return True
